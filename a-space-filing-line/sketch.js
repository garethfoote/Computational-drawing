// Higher values mean greater radius change
let randMin = 1
let randMax = 5
let gridSize = 80
let padding = 100
...
function draw(){
  ...
  //   let newLines = aSpaceFilingLine(allLines, [x-gridSize/2, y-gridSize/2], dash, gap, random(0.005, 0.02))
  ...
  // let newLines = aSpaceFilingLine(allLines, [400, 400], 10, 10, 0.02)
  // allLines = [...newLines, ...allLines];
  // newLines = aSpaceFilingLine(allLines, [padding, padding], random(3, 20), 20, 0.02);
  // allLines = [...newLines, ...allLines];
...
function aSpaceFilingLine(allPreviousLines, startV, dash, gap, noiseMultiplier){
  ...
  while(anyIntersections === false){
    ...
    if(lineIntersections(allPreviousLines, currV.x, currV.y, newX, newY).length > 0){
      ...
      if(debug){
        ...
        simpleTriangle(newX, newY, 15, "down")
        myLine.push([currV.x, currV.y, newX, newY, i<=1])
        console.log("intersects an old line")
      }
    } else if(lineIntersections(myLine, currV.x, currV.y, newX, newY).length > 0){
      anyIntersections = true
      if(debug){
        // Intersects itself
        fill(cl)
        simpleTriangle(newX, newY, 15)
        myLine.push([currV.x, currV.y, newX, newY, i<=1])
